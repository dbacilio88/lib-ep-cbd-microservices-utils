...
import pe.mil.microservices.utils.components.validations.soap.ISoapBusinessRequestValidation;
import pe.mil.microservices.utils.components.validations.soap.ISoapStandardDefinitionValidation;
...
import pe.mil.microservices.utils.service.soap.contracts.ISoapStandardProviderService;
...
public class SoapStandardProviderService extends SoapBaseService implements ISoapStandardProviderService {
    ...
    public <T, R> Mono<SoapBusinessResponse<R>> doOnExecuteSoapService(SoapBusinessRequest<T, R> soapBusinessRequest, ISoapStandardDefinition soapStandardDefinition) {
        ...
        if (ValidateResult.NOT_VALID.equals(soapBusinessRequestValidationResult.getValidateResult())) {
            return Mono.error(() -> new SoapProcessException("error in soap business request validation", ResponseCode.ERROR_DATA_INVALID));
        }
        ...
        if (ValidateResult.NOT_VALID.equals(soapDefinitionValidationResult.getValidateResult())) {
            return Mono.error(() -> new SoapProcessException("error in soap standard definition validation", ResponseCode.ERROR_DATA_INVALID));
        }
            ...
            .flatMap(currentSoapRequest -> {
                ...
                return Mono.just(soapStandardDefinition.validateSoapRequest())
                    .flatMap(currentSoapValidationRequest -> {
                        ...
                        return Mono.error(() -> new SoapProcessException("error in soap request validation, ", ResponseCode.ERROR_DATA_INVALID));
                    });
            })
            .doOnSuccess(success ->
    ...
    private <T> Mono<?> doOnExecuteSoapMessage(final T soapRequest, final ISoapStandardDefinition soapStandardDefinition) {
        ...
        return Mono.just(soapRequest)
            .flatMap(currentSoapRequest -> Mono.just(soapStandardDefinition.buildNovoSoapClientRequest(currentSoapRequest))
