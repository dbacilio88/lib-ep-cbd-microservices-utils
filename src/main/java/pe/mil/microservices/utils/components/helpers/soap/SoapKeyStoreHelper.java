package pe.mil.microservices.utils.components.helpers.soap;
...
public class SoapKeyStoreHelper {
    ...
    public static KeyStore createKeyStore(final String keyStorePath, final String keyStorePassword) {
        ...
        try (InputStream inputStream = new FileInputStream(ResourceUtils.getFile(keyStorePath))) {
            ...
            throw new SoapProcessException("error in process createKeyStore, KeyStoreException", ResponseCode.INTERNAL_SERVER_ERROR);
        } catch (NoSuchAlgorithmException e) {
            log.error("error in process createKeyStore, NoSuchAlgorithmException, error: {}", e.getMessage());
            throw new SoapProcessException("error in process createKeyStore, NoSuchAlgorithmException", ResponseCode.INTERNAL_SERVER_ERROR);
        } catch (CertificateException e) {
            log.error("error in process createKeyStore, CertificateException, error: {}", e.getMessage());
            throw new SoapProcessException("error in process createKeyStore, CertificateException", ResponseCode.INTERNAL_SERVER_ERROR);
        } catch (IOException e) {
            log.error("error in process createKeyStore, IOException, error: {}", e.getMessage());
            throw new SoapProcessException("error in process createKeyStore, IOException", ResponseCode.INTERNAL_SERVER_ERROR);
        } catch (Exception e) {
            log.error("error in process createKeyStore, Exception error: {}", e.getMessage());
            throw new SoapProcessException(e.getMessage());
        }
    }
}
